...
┌──────────────────┐
│ Return Address   │ ← Attacker can overwrite this
├──────────────────┤
│ Saved RBP        │
├──────────────────┤
...
│ Saved RBP        │
├──────────────────┤
│ 🕯️ CANARY VALUE  │ ← Random value checked before return
├──────────────────┤
│ buffer[16]       │ ← Overflow hits canary first!
└──────────────────┘
```
...
3. If canary changed → buffer overflow detected → program terminates
...
┌─────────────────────────────┐
│ Code Segment    [R-X]       │ ← Can read & execute (run programs)
├─────────────────────────────┤
│ Data Segment    [RW-]       │ ← Can read & write (store data)
├─────────────────────────────┤
│ Stack           [RW-]       │ ← Can read & write (NO EXECUTE!)
└─────────────────────────────┘
...
│ Data Segment    [RW-]       │
├─────────────────────────────┤
│ Stack           [RWX]       │ ← Can read, write, AND EXECUTE!
└─────────────────────────────┘
...
# ↑ But loaded at DIFFERENT addresses each run!
```
...
# WITH -no-pie: ✓ This address works every time!
# WITHOUT -no-pie: ✗ Address changes each run, exploit fails!
```
...
# ↑ Just addresses, no function names!
```
...
- ✓ NX prevents stack code execution  
- ✓ PIE/ASLR randomizes addresses
- ✓ Optimizations may add extra security
...
# Example output:
# RELRO:    Partial RELRO
# Stack:    No canary found          ← Our -fno-stack-protector
# NX:       NX disabled              ← Our -z execstack  
# PIE:      No PIE                   ← Our -no-pie
```
